...
func (tx *Tx) ZMembers(bucket string, key []byte) (map[*SortedSetMember]struct{}, error) {
	...
	members, err := tx.db.Index.sortedSet.get(bucket, tx.db).ZMembers(string(key))
	if err != nil {
		return nil, err
	}
...
func (tx *Tx) ZCard(bucket string, key []byte) (int, error) {
	...
	card, err := tx.db.Index.sortedSet.get(bucket, tx.db).ZCard(string(key))
	if err != nil {
		return 0, err
	}
	return card, nil
...
func (tx *Tx) ZCount(bucket string, key []byte, start, end float64, opts *GetByScoreRangeOptions) (int, error) {
	...
	count, err := tx.db.Index.sortedSet.get(bucket, tx.db).ZCount(string(key), SCORE(start), SCORE(end), opts)
	if err != nil {
		return 0, err
	}
...
func (tx *Tx) ZPopMax(bucket string, key []byte) (*SortedSetMember, error) {
	...
	record, score, err := tx.db.Index.sortedSet.get(bucket, tx.db).ZPeekMax(string(key))
	if err != nil {
		return nil, err
	}
...
func (tx *Tx) ZPopMin(bucket string, key []byte) (*SortedSetMember, error) {
	...
	record, score, err := tx.db.Index.sortedSet.get(bucket, tx.db).ZPeekMin(string(key))
	if err != nil {
		return nil, err
	}
...
func (tx *Tx) ZRangeByScore(bucket string, key []byte, start, end float64, opts *GetByScoreRangeOptions) ([]*SortedSetMember, error) {
	...
	records, scores, err := tx.db.Index.sortedSet.get(bucket, tx.db).ZRangeByScore(string(key), SCORE(start), SCORE(end), opts)
	if err != nil {
		return nil, err
	}
...
func (tx *Tx) ZRangeByRank(bucket string, key []byte, start, end int) ([]*SortedSetMember, error) {
	...
	records, scores, err := tx.db.Index.sortedSet.get(bucket, tx.db).ZRangeByRank(string(key), start, end)
	if err != nil {
		return nil, err
	}
...
func (tx *Tx) ZRem(bucket string, key []byte, value []byte) error {
	...
	exist, err := tx.db.Index.sortedSet.get(bucket, tx.db).ZExist(string(key), value)
	if err != nil {
		return err
	}
...
func (tx *Tx) ZRank(bucket string, key, value []byte) (int, error) {
	...
	return tx.db.Index.sortedSet.get(bucket, tx.db).ZRank(string(key), value)
}
...
func (tx *Tx) ZRevRank(bucket string, key, value []byte) (int, error) {
	...
	return tx.db.Index.sortedSet.get(bucket, tx.db).ZRevRank(string(key), value)
}
...
func (tx *Tx) ZScore(bucket string, key, value []byte) (float64, error) {
	...
	if score, err := tx.db.Index.sortedSet.get(bucket, tx.db).ZScore(string(key), value); err != nil {
		return 0, err
	} else {
		return score, nil
	}
...
func (tx *Tx) ZKeys(bucket, pattern string, f func(key string) bool) error {
	...
	for key := range tx.db.Index.sortedSet.get(bucket, tx.db).M {
