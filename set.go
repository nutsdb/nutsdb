...
func (s *Set) SAdd(key string, values [][]byte, records []*data.Record) error {
	set, ok := s.M[key]
	if !ok {
		s.M[key] = map[uint32]*data.Record{}
		set = s.M[key]
	}
...
func (s *Set) SPop(key string) *data.Record {
	if !s.SHasKey(key) {
		return nil
	}
...
func (s *Set) SDiff(key1, key2 string) ([]*data.Record, error) {
...
func (s *Set) SInter(key1, key2 string) ([]*data.Record, error) {
	if !s.SHasKey(key1) || !s.SHasKey(key2) {
		return nil, ErrSetNotExist
	}
...
func (s *Set) SMembers(key string) ([]*data.Record, error) {
	if _, ok := s.M[key]; !ok {
		return nil, ErrSetNotExist
	}
...
func (s *Set) SMove(key1, key2 string, value []byte) (bool, error) {
	...
	if _, ok = set2[hash]; !ok {
		err = s.SAdd(key2, [][]byte{value}, []*data.Record{member})
		if err != nil {
			return false, err
		}
	}
...
func (s *Set) SUnion(key1, key2 string) ([]*data.Record, error) {
	if !s.SHasKey(key1) || !s.SHasKey(key2) {
		return nil, ErrSetNotExist
	}
