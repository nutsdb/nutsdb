...
func (dll *DoublyLinkedList) InsertRecord(key []byte, record *core.Record) bool {
	...
	front := dll.list.Front().Value.(*Item[core.Record])
	back := dll.list.Back().Value.(*Item[core.Record])
...
func (dll *DoublyLinkedList) Min() (*Item[core.Record], bool) {
	front := dll.list.Front()
	if front == nil {
		return nil, false
	}
	elem := front.Value.(*Item[core.Record])
	return &Item[core.Record]{
		Key:    elem.Key,
		Record: elem.Record,
	}, true
}
...
func (dll *DoublyLinkedList) Max() (*Item[core.Record], bool) {
	back := dll.list.Back()
	if back == nil {
		return nil, false
	}
	elem := back.Value.(*Item[core.Record])
	return &Item[core.Record]{
		Key:    elem.Key,
		Record: elem.Record,
	}, true
...
func (dll *DoublyLinkedList) PopMin() (*Item[core.Record], bool) {
	front := dll.list.Front()
	if front == nil {
		return nil, false
	}
...
func (dll *DoublyLinkedList) PopMax() (*Item[core.Record], bool) {
	back := dll.list.Back()
	if back == nil {
		return nil, false
	}
...
func (dll *DoublyLinkedList) AllItems() []*Item[core.Record] {
	items := make([]*Item[core.Record], 0, dll.list.Len())
	for e := dll.list.Front(); e != nil; e = e.Next() {
		elem := e.Value.(*Item[core.Record])
		items = append(items, &Item[core.Record]{
			Key:    elem.Key,
			Record: elem.Record,
		})
	}
