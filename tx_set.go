...
func (tx *Tx) sPut(bucket string, key []byte, dataFlag uint16, values ...[]byte) error {
	...
	if dataFlag == core.DataSetFlag {
		...
		b1, err := tx.db.bm.GetBucket(core.DataStructureSet, bucket)
		if err != nil {
			return err
		}
		bucketId := b1.Id
...
func (tx *Tx) SRem(bucket string, key []byte, items ...[]byte) error {
	...
	b, err := tx.db.bm.GetBucket(core.DataStructureSet, bucket)
	if err != nil {
		return err
	}
	bucketId := b.Id
...
func (tx *Tx) SDiffByTwoBuckets(bucket1 string, key1 []byte, bucket2 string, key2 []byte) ([][]byte, error) {
	...
	b1, err := tx.db.bm.GetBucket(core.DataStructureSet, bucket1)
	if err != nil {
		return nil, err
	}
	bucketId1 := b1.Id

	b2, err := tx.db.bm.GetBucket(core.DataStructureSet, bucket2)
	if err != nil {
		return nil, err
	}
	bucketId2 := b2.Id
