...
func TestTx_DeleteFromMemory(t *testing.T) {
	runNutsDBTest(t, nil, func(t *testing.T, db *DB) {
		...
		err := db.View(func(tx *Tx) error {
			r, ok := db.Index.bTree.get(bucket).Find(GetTestBytes(3))
			require.Nil(t, r)
			require.False(t, ok)
...
func TestTx_ExpiredDeletion(t *testing.T) {
	...
	t.Run("expired deletion", func(t *testing.T) {
		runNutsDBTest(t, nil, func(t *testing.T, db *DB) {
			...
			r, ok := db.Index.bTree.get(bucket).Find(GetTestBytes(0))
			require.Nil(t, r)
			require.False(t, ok)

			r, ok = db.Index.bTree.get(bucket).Find(GetTestBytes(1))
			require.Nil(t, r)
			require.False(t, ok)
		})
	})
	...
	t.Run("expire deletion when merge", func(t *testing.T) {
		...
		runNutsDBTest(t, &opts, func(t *testing.T, db *DB) {
			...
			r, ok := db.Index.bTree.get(bucket).Find(GetTestBytes(2))
			require.Nil(t, r)
			require.False(t, ok)
		})
	})
